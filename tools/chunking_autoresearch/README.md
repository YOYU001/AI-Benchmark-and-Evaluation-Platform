...
- **`dashboard.py`** — 本機即時儀表板，讀 `results.tsv` 畫出每項指標的
  趨勢圖跟成長百分比，用瀏覽器自己的重新整理看最新結果（曾經是每 5 秒自動
  整頁重新整理，但圖表變寬、變複雜之後很明顯地閃爍，已改掉）。
...
第二版加了**硬性門檻**（後來又補了兩道，詳見 `program.md` 裡「2026-08-14 補」
「2026-08-14 補二」兩段的完整說明）：

1. `quality_pass_rate` 必須是 1.0（欄位格式全部合法）
2. `content_coverage` 必須 ≥ 0.90（用固定長度字元 shingle 比對，原始文件
   內容至少要有 90% 真的出現在切出來的 chunk 裡）
3. 單一 chunk 不能佔同一份文件超過 50% 的字元數（`MAX_CHUNK_SHARE_OF_DOCUMENT`，
   擋「整份文件包成一個 chunk」這種取巧做法）
4. 單一 chunk 的絕對字元數不能超過 2000（`MAX_CHUNK_CHAR_COUNT`，擋「無腦
   對半切」這種比例上卡在門檻 3 以內、但一樣沒做到有意義切分的取巧做法）

四個門檻都過，`cost_time` 才是「執行時間相對 baseline 的倍數」，這時候才是
真正在比速度。只要有一個沒過，`cost_time` 直接跳到 1000 以上，保證輸給任何
有認真切的策略——「丟資料換速度」跟「不做有意義切分換速度」這兩條路都已經
被堵死。
...
瀏覽器打開 `http://127.0.0.1:8765/`，用瀏覽器自己的重新整理看最新結果。畫面內容：
...
結果時，`results.tsv` 每次請求都會重新讀取，重新整理頁面就能看到最新結果，
不用重啟伺服器。
