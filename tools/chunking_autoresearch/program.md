...
9. 如果 `cost_time` 一樣或變差，或是 `hard_gate_passed` 是 `False`（status 記
   ...
   **這個 `git reset --hard` 已經事先被人類明確授權，範圍僅限於這個
   worktree（`chunking-autoresearch/<tag>` 分支）、目標僅限於步驟 1 記下的
   `attempt_base`（也就是這一輪自己造成的變更，不會動到這一輪以前的任何
   commit）。不需要每次都額外用 clarify／詢問人類確認才執行——這正是整個
   自主迴圈設計「有效就留、沒效就丟」的核心機制，每次都停下來等人核准就
   不是自主研究了。真的需要人類介入的情況，只有「停下來的條件」那節列出
   的那幾種，不包含這個例行的 reset 動作。**
