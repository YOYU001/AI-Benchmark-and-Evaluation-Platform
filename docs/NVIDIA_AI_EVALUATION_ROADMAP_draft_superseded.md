...
本學習 Roadmap 將以 **NVIDIA 目前在 AI Evaluation、Benchmarking、RAG Evaluation、Agent Evaluation 與 Performance Benchmarking 所採用的主流方法與工具** 作為主要學習目標。

學習重點不是單純熟悉某一個模型或某一套工具，而是逐步建立一套可以長期擴充、重複使用、並能實際支援 AI 導入決策的 Evaluation 方法與自動化系統。

整體學習與系統設計將圍繞以下 **兩大核心面向**：
...
目的不是只看某個公開 Benchmark 的分數，而是建立一份完整的 AI 能力與工程表現 Scorecard，回答：

> 這個 AI 整體而言有哪些優勢、弱點、適合用途與不適合用途？
...
這兩個面向將作為整份 Roadmap、未來 Evaluation Lab / EvalCore，以及所有自動化 Evaluation Pipeline 的核心設計原則。
...
模型答案與 Ground Truth 是否一致。

## Context Relevancy

Retriever 找回來的 chunk 是否真的與問題相關。

## Response Groundedness

回答是否有被 retrieved context 支持，而不是模型自己亂補。

## Context Recall

檢查正確資料是否成功被 Retrieval 找回。
...
即使 Overall Score 上升，也不能直接 deployment。
...
最終它可以成為一個獨立 Portfolio Project，也可以作為其他 AI 專案共用的 Evaluation Infrastructure。
