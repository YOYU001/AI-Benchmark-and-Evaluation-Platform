...
- **NAS 上獨立啟動 `dashboard.py`，並用 git worktree 隔離 Hermes 的實驗迴圈跟 dashboard 的 checkout**：
  - `dashboard.py` 原本綁死 `127.0.0.1`（只能本機存取），改成讀 `DASHBOARD_HOST` 環境變數決定監聽位址，預設值不變、不影響既有本機用法。docker-compose 新增獨立的 `chunking-dashboard` 服務（同一份 image，`command: ["python3", "dashboard.py"]`，`DASHBOARD_HOST=0.0.0.0`），一開始想把 port 綁定在 NAS 的 Tailscale IP（`100.92.223.84:8765:8765`）失敗（`driver failed programming external connectivity`，Tailscale 虛擬網卡跟 Docker 連接埠轉發機制不相容），改成不指定 host IP（`8765:8765`，監聽所有介面）後成功。
  - 發現 Hermes 排程跟 dashboard 共用同一份 NAS checkout 資料夾的潛在風險：cron 執行時會把該資料夾切到實驗分支，若 dashboard 容器剛好在那個時間點重啟，會讀到舊版（沒有 `DASHBOARD_HOST` 支援）的 `dashboard.py`，導致外部連線失效且不易察覺——跟先前 Windows 端「Claude Code 與 Hermes 共用一個工作目錄互相干擾」是同一類問題，解法比照沿用：讓 Hermes 用 `git worktree add` 建立獨立資料夾給實驗迴圈用，`AI-Benchmark-and-Evaluation-Platform` 本身固定停在 `main`、只給 dashboard 讀。
  - 容器內 `/workspace/projects/` 這一層沒有寫入權限，無法把 worktree 建在專案資料夾外面（同層），改成建在專案資料夾內部的 `.worktrees/chunking-autoresearch/`（git 支援的標準巢狀 worktree 做法，效果相同：該子資料夾切換分支不影響外層 checkout），並把 `.worktrees/` 加進 `.gitignore` 避免污染 `git status`（commit `195c791`）。
  - 排程的 `--workdir` 跟 prompt 內文都同步改成指向 `.worktrees/chunking-autoresearch`（先前只改了 `--workdir` 執行參數，prompt 文字仍寫著舊路徑，Hermes 自己發現這個不一致並主動提出要不要一併修正，已修）。
- **幫 NAS 上的 Hermes 設定 GitHub 推送權限**：容器內原本沒有任何 git 認證，`git push` 會因為非互動環境無法輸入帳密而失敗。申請 GitHub **Fine-grained Personal Access Token**（範圍限定在這個 repo、只給 Contents Read/write，比 classic token 全帳號通用權限更安全），Hermes 用 `git config --global credential.helper store` 存起來，之後 `git push origin main` 不用再帶 token。
- **移除又重建了一次既有的 Google Drive 每日備份排程**（`AI 每日備份至 Google Drive`，這是使用者先前就設定好的排程，非本次新增）：
  - 一開始使用者決定資料留在 NAS 本地就好、不需要雲端備份，請 Hermes 移除該排程前先說明備份內容（個人設定、金鑰/登入狀態、technic skills/cron/memories 三個資料夾、對話 state.db，打包 zip → AES 加密 → 上傳 Google Drive「AI 備份」資料夾、只留最近 10 份）；查出前一晚（08/11 03:00）失敗原因是環境缺少 `pyzipper` 套件、卡在加密步驟，前面打包本身是成功的（13.87MB）。
